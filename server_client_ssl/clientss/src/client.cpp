...
void client::init(){
    ...
    const int sfd = OpenConnection("127.0.0.1", "8887");
    SSL_set_fd(ssl, sfd);
...
int client::SSL_read_all(SSL *ssl, void* buf, int num)
{
...
void client::DisplayCerts(SSL *ssl)
{
...
int client::OpenConnection(const char *hostname, const char *port)
{
   struct hostent *host;
...
SSL_CTX * client::InitSSL_CTX(void)
{
